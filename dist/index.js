...
(() => {
  ...
  // src/animations/cardStack.ts
  var initCardStack = () => {
    gsapWithCSS.registerPlugin(ScrollTrigger2);
    const cards = gsapWithCSS.utils.toArray(".card");
    if (cards.length === 0) {
      return;
    }
    ...
    const tl = gsapWithCSS.timeline({
      scrollTrigger: {
        ...
        start: "center center",
        // Start when section is centered in viewport
        end: `+=${totalCards * 120}`,
        // Duration based on number of cards
        scrub: 0.8,
        pin: true,
        // Pin the entire section during animation
        anticipatePin: 1
      }
    });
    cards.forEach((card, i) => {
  ...
  // src/animations/revealAnimations.ts
  var initRevealAnimations = () => {
    gsapWithCSS.registerPlugin(ScrollTrigger2);
    document.body.classList.add("gsap-ready");
    const revealElements = gsapWithCSS.utils.toArray(".reveal:not(.card)");
    const fadeUpElements = gsapWithCSS.utils.toArray(".fade-up:not(.card)");
    gsapWithCSS.set(revealElements, { opacity: 0, y: 50, force3D: true });
    gsapWithCSS.set(fadeUpElements, { opacity: 0, y: 80, force3D: true });
    ScrollTrigger2.batch(revealElements, {
      onEnter: (elements) => {
    ...
    ScrollTrigger2.batch(fadeUpElements, {
      onEnter: (elements) => {
        gsapWithCSS.to(elements, {
          opacity: 1,
          y: 0,
